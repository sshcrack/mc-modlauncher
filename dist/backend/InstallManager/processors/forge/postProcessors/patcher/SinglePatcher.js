...
class SinglePatcher extends Processor_1.ProcessEventEmitter {
    ...
    getLogger() {
        return mainLogger_1.MainLogger.get("InstallManager", "processors", "forge", "postProcessors", "patcher", "SinglePatcher", this.options.jar);
    }
    run() {
        return new Promise((resolve, reject) => {
            const logger = this.logger;
            const fileLogger = (0, file_1.getStdoutFile)(this.options.jar);
            fs_1.default.writeFileSync(fileLogger, "");
            ...
            child.stderr.on("data", (err) => {
                ...
                stderr += err;
                fs_1.default.appendFileSync(fileLogger, `ERR: ${err.toString()}`);
            });
            child.stdout.on("data", (chunk) => {
                stdout += chunk;
                const length = stdout.length;
                const part = stdout.substring(length - 100);
                fs_1.default.appendFileSync(fileLogger, chunk.toString());
