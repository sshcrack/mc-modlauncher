...
*/
const toLocaleString = (number: number, locale: boolean, options: unknown) => {
    let result = number as unknown as string;
...
export default function prettyBytes(number: number, options?: Options) {
    ...
    if (number < 1) {
        const numberString = toLocaleString(number, options.locale as unknown as boolean, localeOptions);
    ...
    if (!localeOptions) {
        number = parseInt(number.toPrecision(3));
    }

    const numberString = toLocaleString(Number(number), options.locale as unknown as boolean, localeOptions);
